#include "Matrix.h"
matrix::matrix(int rows, int cols) : //конструстор создающий пустую матрицу с нужным количеством строк и столбцов;
	_rows(rows),
	_cols(cols)
{
...
//перегрузка оператора вывода
 ostream& operator <<(ostream& out, const matrix& mtx) { // стандартная штука, friend- с помощью него мы получаем доступ к полям класса
 ...
 istream& operator >>(istream& in, matrix& mtx) { // ввод в консоль матрицы;
	for (int i = 0; i < mtx._rows; i++) {
		for (int j = 0; j < mtx._cols; j++)
			in >> mtx._data[i][j];
	}
...
//сложение матриц 
 matrix operator+(matrix& leftop, matrix rightop) { // проверка 
...
matrix& matrix::operator+=(matrix mtx) { // проверка 
...
matrix operator*(matrix& mtx, double s) {
...
matrix operator*(double s, matrix& mtx) {
	matrix res(mtx._rows, mtx._cols);
	for (int i = 0; i < res._rows; i++)
		for (int j = 0; j < res._cols; j++)
			res._data[i][j] = mtx._data[i][j] * s;
...
matrix matrix::mulByEl(const matrix& right) {     // создаем правую матрицу, а левую будем вызывать
...
matrix matrix::divByEl(const matrix& right) {
	if (_rows != right._rows || _cols != right._cols)
		throw logic_error("Matrix must be same size");
	matrix res(_rows, _cols);
	for (int i = 0; i < res._rows; i++)
...
//оператор возведения в степень матрицы
 matrix operator^(const matrix& mtx, int s) {
	if (!mtx.isSquare())
...
//вспомогательная функция для получения обратной матрицы
 matrix matrix:: E(int size) {        //метод отвязан от объекта, возвращает матрицу размера size*size
	matrix e(size, size);
...
//2- бесконечная норма 
//3- сферическая норма 
double matrix::norm(int i) const {
	if (i == 1)
	{
		double max = -1;
...
double& matrix::operator ()(int i, int j) //работает с неконстантными объектами 
{
	if (i >= _rows || j >= _cols)
		throw logic_error("Out of range index operator()");
	return _data[i][j];
}
double matrix::operator ()(int i, int j) const     //работает с константными объектами 
{
	if (i >= _rows || j >= _cols)
		throw logic_error("Out of range index operator()");
	return _data[i][j];
}
int matrix::rows()const { //возращает размер сколько строк 
	return _rows;
}
int matrix::cols()const { //возвращает сколько столбцов
	return _cols;
}
vector<double> matrix::get_row(int i) const {                      //возвращает i строку 
	if (i >= _rows)
		throw logic_error("Out of range index get_row");
	return _data[i];
}

vector<double> matrix::get_col(int j) const {                       //возвращает j столбец
	if (j >= _cols)
